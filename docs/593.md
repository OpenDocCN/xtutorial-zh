...
在这个例子中，样式只影响这个特定的 文本块控件，何必呢？好吧，在这种情况下，这是毫无意义的。我本可以用 文本块控件上的一个 FontSize 属性来替换所有这些额外的标签，但是正如我们将在后面看到的那样，样式可以做的不仅仅是设置属性，例如，样式触发器可以使上面的例子在实际应用中有用。然而，您将定义的大多数样式可能会在更高的范围内。
